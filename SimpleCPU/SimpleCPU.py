...
class State(rx.State):
	...
	async def upload_progmem(self, files: list[rx.UploadFile]):
		...
		raw_code = data.decode()
		self.progmem = code_formatting(raw_code)
		self.state = "running"

	@rx.event
	def submit_progmem(self, data: dict):
		self.cpu_reset()
		lines = data["text_area"]
		self.progmem = code_formatting("\n".join(lines))
		self.state = "running"
...
def code_formatting(raw: str) -> list[str]:
	result = []
	for line in raw.split("\n"):
		parts = line.split(" ")

		# blank space (tv) deletion
		while "" in parts:
			parts.remove("")

		clean = " ".join(parts)

		# blank line deletion condition
		if clean:
			result.append(clean)
